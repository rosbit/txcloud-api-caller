...
func checkMust(confFile string) error {
	...
	if len(ak.SecretId) == 0 {
		return fmt.Errorf("access-key/secret-id expected in conf")
	}
	if len(ak.SecretKey) == 0 {
		return fmt.Errorf("access-key/secret-key expected in conf")
	}
